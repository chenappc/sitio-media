...
export const metadata = {
  title: "Stories",
  description: "Stories on Vahica.com",
};

export default async function StoriesEnPage() {
  const stories: Story[] = await getStories();
  const publicadas = stories.filter((s) => s.status === "published" && s.idioma === "en");

  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="font-serif text-2xl font-bold mb-8">Stories</h1>
      {publicadas.length === 0 ? (
        <p className="text-[var(--negro)]/60">No stories published yet.</p>
      ) : (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {publicadas.map((story) => (
            <Link
              key={story.id}
              href={`/en/stories/${story.slug}/1`}
            ...
            >
              <div className="relative aspect-[4/3] w-full bg-[var(--negro)]/5">
                {(story as any).imagen_portada ? (
                  ...
                  <div className="flex h-full items-center justify-center text-[var(--negro)]/20 text-sm">
                    No image
                  </div>
                )}
              </div>
